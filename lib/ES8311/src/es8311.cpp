...
#include "esp_err.h"
#include "esp_check.h"
// driver/i2s_std.h intentionally removed -- not part of this toolchain's
// ESP-IDF version, and unused anywhere in this file (no i2s_std_*/
// i2s_chan_* symbols referenced) -- likely a leftover include from a
// fuller reference implementation Freenove based this file on.
...
esp_err_t es8311_codec_init(void)
{
     ...
     * NOTE (Grey's translate project): mclk_from_mclk_pin is set to false
     * here, unlike Freenove's original sketch. Their sketch used the newer
     * ESP_I2S.h library which drives a dedicated MCLK output pin; our
     * toolchain only has the older bundled I2S.h library, which does not
     * expose a configurable MCLK pin. es8311_init() derives the codec's
     * internal clock from the SCLK/BCLK timing instead when this is false
     * (see clk_cfg->mclk_from_mclk_pin branch above), which is the
     * documented alternative to an external MCLK signal. */
    es8311_handle_t es_handle = es8311_create(I2C_NUM_0, ES8311_ADDRRES_0);
    ESP_RETURN_ON_FALSE(es_handle, ESP_FAIL, TAG, "es8311 create failed");
    /* Reverted to mclk_from_mclk_pin = true (matching Freenove's actual
     * verified config for this board -- confirmed via a working ESPHome
     * project for the same FNK0104AB hardware, which uses use_mclk: true
     * on GPIO4). The earlier assumption that the older Arduino I2S.h
     * library couldn't drive a separate MCLK pin was wrong -- the legacy
     * ESP-IDF driver/i2s.h it wraps DOES support mck_io_num, the Arduino
     * wrapper class just never exposes it. main.cpp now calls
     * i2s_set_pin() directly (bypassing the wrapper) right after
     * I2S.begin() to add real MCLK output on GPIO4. */
     ...
     * Raised from 24DB to the max (42DB) on 2026-08-23 -- a quieter voice
     * (Grey's daughter) wasn't being picked up at all even after being
     * asked to speak louder. */
    ...
    // es8311_voice_mute() at all, so if the chip resets muted the speaker
    // would stay silent even though every other step reports success.
